		...
		QRProof prove(
			...
			for (size_t i = 0; i < NumInputs; i++) {
				// Sanity checks of input
				{
					...
					if (inputs[i].note.assetID == assetID) {
			...
			pb.get_constraint_system().swap_AB_if_beneficial();
...
//#define DEBUG_LOGS
#ifdef DEBUG_LOGS
			{
				std::ofstream out("primary_input.key", std::ofstream::binary);
				out << primary_input;
				out.close();
			}

			{
				std::ofstream out("aux_input.key", std::ofstream::binary);
				out << aux_input;
				out.close();
			}
#endif // DEBUG_LOGS
			...
			r1cs_ppzksnark_proof<ppzksnark_ppT> proof = r1cs_ppzksnark_prover<ppzksnark_ppT>(pk, primary_input, aux_input);

			const bool ans = r1cs_ppzksnark_verifier_strong_IC<ppzksnark_ppT>(vk, primary_input, proof);

			if (ans == false) {
				throw std::runtime_error(strprintf("Cannot generate the proof"));
			}

			printf("%d", ans);
			return QRProof(proof);
			//return QRProof(libsnark::r1cs_ppzksnark_prover<ppzksnark_ppT>(
			//	pk,
			//	primary_input,
			//	aux_input
			//	));
	...
	JSOutput::JSOutput() : addr(uint256(), uint256()), value(0) {
		SpendingKey a_sk = SpendingKey::random();
		addr = a_sk.address();
		assetID = random_uint256();
	}

	JSInput::JSInput(uint256 assetID) : witness(QRIncrementalMerkleTree().witness()),
		key(SpendingKey::random()) {
		note = Note(key.address().a_pk, 0, random_uint256(), random_uint256(), assetID);
		QRIncrementalMerkleTree dummy_tree;
		dummy_tree.append(note.cm());
		witness = dummy_tree.witness();
	}

	JSInput::JSInput() : witness(QRIncrementalMerkleTree().witness()),
		key(SpendingKey::random()) {
		note = Note(key.address().a_pk, 0, random_uint256(), random_uint256(), random_uint256());
		QRIncrementalMerkleTree dummy_tree;
		dummy_tree.append(note.cm());
		witness = dummy_tree.witness();
	}

	template class JoinSplit<QR_NUM_JS_INPUTS, QR_NUM_JS_OUTPUTS>;
