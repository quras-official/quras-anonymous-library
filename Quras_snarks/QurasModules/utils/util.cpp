...
boost::filesystem::path GetDefaultDataDir()
{
	...
	// Windows < Vista: C:\Documents and Settings\Username\Application Data\Quras
	// Windows >= Vista: C:\Users\Username\AppData\Roaming\Quras
	// Mac: ~/Library/Application Support/Quras
	// Unix: ~/.quras
#ifdef WIN32
	// Windows
	return GetSpecialFolderPath(CSIDL_APPDATA) / "Quras";
#else
	fs::path pathRet;
...
static boost::filesystem::path ZC_GetBaseParamsDir()
{
...
#ifdef WIN32
	// Windows
	return GetSpecialFolderPath(CSIDL_APPDATA) / "QurasParams";
#else
	fs::path pathRet;
	char* pszHome = getenv("HOME");
	if (pszHome == NULL || strlen(pszHome) == 0)
	...
	pathRet /= "Library/Application Support";
	TryCreateDirectory(pathRet);
	return pathRet / "QurasParams";
#else
	// Unix
	return pathRet / ".quras-params";
#endif
#endif
}
