...
private:
	...
	std::shared_ptr<digest_variable<FieldT>> zk_assetID;
...
public:
	...
	joinsplit_gadget(protoboard<FieldT> &pb) : gadget<FieldT>(pb) {
		// Verification// Verification
		{
			...
			alloc_uint256(zk_unpacked_inputs, zk_assetID);
	...
	void generate_r1cs_constraints() {
		...
		zk_assetID->generate_r1cs_constraints();
		...
		// Value balance
		{
			...
			linear_combination<FieldT> left_assetid = packed_addition_256(zk_assetID->bits);
			for (size_t i = 0; i < NumInputs; i++) {
				linear_combination<FieldT> right_assetid = packed_addition_256(zk_input_notes[i]->assetID->bits);
			...
			for (size_t i = 0; i < NumOutputs; i++) {
				linear_combination<FieldT> right_assetid = packed_addition_256(zk_output_notes[i]->assetID->bits);
				this->pb.add_r1cs_constraint(r1cs_constraint<FieldT>(
					1,
					left_assetid,
					right_assetid
	...
	void generate_r1cs_witness(
		...
		zk_assetID->bits.fill_with_bits(
			this->pb,
			uint256_to_bool_vector(assetID)
		);
...

		{
			...
			for (size_t i = 0; i < NumInputs; i++) {
				if (assetID == inputs[i].note.assetID) {
	...
	static r1cs_primary_input<FieldT> witness_map(
		...
		insert_uint256(verify_inputs, assetID);
