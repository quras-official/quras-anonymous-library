...
template<typename T>
T swap_endianness_u256(T v) {
	if (v.size() != 256) {
		throw std::length_error("invalid bit length for 256-bit unsigned integer");
	}
...
template<typename FieldT>
linear_combination<FieldT> packed_addition_256(pb_variable_array<FieldT> input) {
	auto input_swapped = swap_endianness_u256(input);
