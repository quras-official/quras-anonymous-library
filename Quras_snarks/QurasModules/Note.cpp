...
namespace libquras {
	...
	NotePlaintext::NotePlaintext(
		const Note& note,
		boost::array<unsigned char, QR_MEMO_SIZE> memo) : memo(memo)
	{
		value = note.value;
		rho = note.rho;
		r = note.r;
	...
	NotePlaintext NotePlaintext::decrypt(const QRNoteDecryption& decryptor,
	...
	QRNoteEncryption::Ciphertext NotePlaintext::encrypt(QRNoteEncryption& encryptor,
