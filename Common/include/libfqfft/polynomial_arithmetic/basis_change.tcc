...
void compute_subproduct_tree(const size_t &m, std::vector< std::vector < std::vector <FieldT> > > &T)
{
    ...
    T[0] = std::vector< std::vector<FieldT> >(1ull << m);
    for (size_t j = 0; j < (1ull << m); j++)
    {
    ...
    {
        T[i] = std::vector<std::vector<FieldT> >(1ull << (m-i));
        for (size_t j = 0; j < (1ull << (m-i)); j++)
        {
            a = T[i-1][index];
            index++;
