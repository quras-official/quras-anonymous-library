...
step_radix2_domain<FieldT>::step_radix2_domain(const size_t m) : evaluation_domain<FieldT>(m)
{
    ...
    big_m = 1ull<<(libff::log2(m)-1);
    small_m = m - big_m;

    if (small_m != 1ull<<libff::log2(small_m))
        throw DomainSizeException("step_radix2(): expected small_m == 1ull<<log2(small_m)");

    try { omega = libff::get_root_of_unity<FieldT>(1ull<<libff::log2(m)); }
...
void step_radix2_domain<FieldT>::FFT(std::vector<FieldT> &a)
{
    ...
    const size_t compr = 1ull<<(libff::log2(big_m) - libff::log2(small_m));
