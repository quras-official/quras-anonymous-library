...
void _basic_serial_radix2_FFT(std::vector<FieldT> &a, const FieldT &omega)
{
    ...
    if (n != (1ull << logn)) throw DomainSizeException("expected n == (1ull << logn)");
...
{
    const size_t num_cpus = 1ull<<log_cpus;

    const size_t m = a.size();
    const size_t log_m = log2(m);
    if (m != 1ull<<log_m) throw DomainSizeException("expected m == 1ull<<log_m");
    ...
    {
        tmp[j].resize(1ull<<(log_m-log_cpus), FieldT::zero());
    }
    ...
    for (size_t j = 0; j < num_cpus; ++j)
    {
        ...
        for (size_t i = 0; i < 1ull<<(log_m - log_cpus); ++i)
        {
    ...
    {
        for (size_t j = 0; j < 1ull<<(log_m - log_cpus); ++j)
        {
...
std::vector<FieldT> _basic_radix2_evaluate_all_lagrange_polynomials(const size_t m, const FieldT &t)
{
    ...
    if (m != (1ull << libff::log2(m))) throw DomainSizeException("expected m == (1ull << log2(m))");
