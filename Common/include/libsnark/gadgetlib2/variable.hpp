...
typedef unsigned long long VarIndex_t;
...
class FElemInterface {
public:
    virtual FElemInterface& operator=(const long long n) = 0;
    ...
    virtual bool operator==(const long long n) const = 0;
    ...
    virtual long long asLong() const = 0;
    virtual int getBit(unsigned int i) const = 0;
    virtual FElemInterface& power(long long exponent) = 0;
...
inline bool operator==(const long long first, const FElemInterface& second) {return second == first;}
inline bool operator!=(const long long first, const FElemInterface& second) {return !(first == second);}
inline bool operator!=(const FElemInterface& first, const long long second) {return !(first == second);}
...
public:
    ...
    FElem& operator=(const long long i) { *elem_ = i; return *this;}
    ...
    long long asLong() const {return elem_->asLong();}
    int getBit(unsigned int i, const FieldType& fieldType);
    friend FElem power(const FElem& base, long long exponent);
...
/// These operators are not always mathematically well defined. The long long will be checked in runtime
/// for fields in which values other than 0 and 1 are not well defined.
inline bool operator==(const FElem& first, const long long second) {return first == FElem(second);}
inline bool operator==(const long long first, const FElem& second) {return second == first;}
inline bool operator!=(const FElem& first, const long long second) {return !(first == second);}
inline bool operator!=(const long long first, const FElem& second) {return !(first == second);}
...
private:
    long long contents_;
    explicit FConst(const long long n) : contents_(n) {}
public:
    virtual FConst& operator=(const long long n) {contents_ = n; return *this;}
    ...
    virtual bool operator==(const long long n) const {return contents_ == n;}
...
public:
    ...
    virtual R1P_Elem& operator=(const long long n) {elem_ = Fp(n); return *this;}
    ...
    virtual bool operator==(const long long n) const {return elem_ == Fp(n);}
    /// @return a unique_ptr to a new copy of the element
    ...
    virtual FElemInterface& power(long long exponent) {elem_^= exponent; return *this;}
...
public:
    ...
    LinearTerm(const Variable& v, long long n) : variable_(v), coeff_(n) {}
...
public:
    ...
    LinearCombination(long long i) : linearTerms_(), constant_(i) {}
