...
FElem::FElem(const long long n) : elem_(new FConst(n)) {}
...
FElem power(const FElem& base, long long exponent) { // TODO .cpp
    FElem retval(base);
...
FElemInterface& FConst::power(long long exponent) {
...
long long R1P_Elem::asLong() const {
    //GADGETLIB_ASSERT(elem_.as_ulong() <= LONG_MAX, "long overflow occured.");
    return long long(elem_.as_ulong());
}
