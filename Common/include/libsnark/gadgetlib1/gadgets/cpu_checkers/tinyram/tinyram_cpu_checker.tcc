    ...
    next_state(next_state), next_pc_addr(next_pc_addr), next_has_accepted(next_has_accepted)
{
    ...
    opcode_indicators.allocate(pb, 1ull<<pb.ap.opcode_width(), FMT(annotation_prefix, " opcode_indicators"));

    /* perform the ALU operations */
    instruction_results.allocate(pb, 1ull<<pb.ap.opcode_width(), FMT(annotation_prefix, " instruction_results"));
    instruction_flags.allocate(pb, 1ull<<pb.ap.opcode_width(), FMT(annotation_prefix, " instruction_flags"));
...
void tinyram_cpu_checker<FieldT>::generate_r1cs_constraints()
{
    ...
    for (size_t i = 0; i < 1ull<<this->pb.ap.opcode_width(); ++i)
    {
                                                              ...
                                                              const tinyram_input_tape_iterator &aux_end)
{
    ...
    else
    {
        ...
        const size_t loaded_word = (prev_doubleword >> (access_is_word0 ? 0 : this->pb.ap.w)) & ((1ull << this->pb.ap.w) - 1);
