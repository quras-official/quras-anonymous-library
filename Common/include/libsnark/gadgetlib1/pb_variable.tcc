...
{
    assert_except(this->size() == vals.size());
...
{
    assert_except(this->size() == bits.size());
...
template<typename FieldT>
void pb_variable_array<FieldT>::fill_with_bits_of_ulong(protoboard<FieldT> &pb, const unsigned long long i) const
{
...
libff::bit_vector pb_variable_array<FieldT>::get_bits(const protoboard<FieldT> &pb) const
{
    ...
    for (size_t i = 0; i < this->size(); ++i)
    {
        ...
        assert_except(v == FieldT::zero() || v == FieldT::one());
...
{
    assert_except(this->is_variable == false);
...
template<typename FieldT>
void pb_linear_combination_array<FieldT>::fill_with_bits_of_ulong(protoboard<FieldT> &pb, const unsigned long long i) const
{
...
{
    assert_except(v.size() == coeffs.size());
    std::vector<linear_term<FieldT> > all_terms;
