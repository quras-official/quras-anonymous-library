...
std::vector<tinyram_instruction> generate_tinyram_prelude(const tinyram_architecture_params &ap)
{
    ...
    const size_t mem_start = 1ull<<(ap.w-1);
...
{
    // remember that memory consists of 1ull<<dwaddr_len() double words (!)
    memory_contents m;
    ...
    const size_t input_addr = 1ull << (dwaddr_len() - 1);
...
{
    const tinyram_opcode opcode = (tinyram_opcode)(std::rand() % (1ull<<ap.opcode_width()));
    const bool arg2_is_imm = std::rand() & 1;
    const size_t desidx = std::rand() % (1ull<<ap.reg_arg_width());
    const size_t arg1idx = std::rand() % (1ull<<ap.reg_arg_width());
    const size_t arg2idx_or_imm = std::rand() % (1ull<<ap.reg_arg_or_imm_width());
                                                             ...
                                                             const tinyram_input_tape &primary_input)
{
    ...
    const size_t primary_input_base_addr = (1ull << (ap.dwaddr_len()-1));
