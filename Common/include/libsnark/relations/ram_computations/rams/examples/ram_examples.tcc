...
ram_example<ramT> gen_ram_example_complex(const ram_architecture_params<ramT> &ap, const size_t boot_trace_size_bound, const size_t time_bound, const bool satisfiable)
{
    ...
    const size_t input_addr = (1ull<<(ap.w-1)) + (ap.w/8); // byte address of the first input word
    ...
    result.boot_trace.set_trace_entry(boot_pos++, std::make_pair(1ull<<(ap.dwaddr_len()-1), satisfiable ? 1ull<<ap.w : 0));

    for (size_t i = 1; i < input_size; ++i)
    {
        result.boot_trace.set_trace_entry(boot_pos++, std::make_pair((1ull<<(ap.dwaddr_len()-1)) + i + 1, std::rand() % (1ull<<(2*ap.w))));
    }
