                        ...
                        const bool test_serialization)
{
	...
	r1cs_ppzksnark_primary_input<ppT> primary_input;
	r1cs_ppzksnark_auxiliary_input<ppT> aux_input;
	{
		std::ifstream ivk("C:\\Users\\dotFund\\Desktop\\work\\primary_input.key", std::ofstream::binary);
		ivk >> primary_input;
		ivk.close();
	}

	{
		std::ifstream ivk("C:\\Users\\dotFund\\Desktop\\work\\aux_input.key", std::ofstream::binary);
		ivk >> aux_input;
		ivk.close();
	}

	{
		std::ifstream ivk("V:\\06_GitHub\\02_bitbucket\\01_Quras\\cryptocurrency\\Quras-gui-wpf\\bin\\Debug\\crypto\\pk.key", std::ofstream::binary);
		ivk >> keypair.pk;
		ivk.close();
	}

    r1cs_ppzksnark_proof<ppT> proof = r1cs_ppzksnark_prover<ppT>(keypair.pk, primary_input, aux_input);

	{
		std::ofstream prooffile("out_proof.key", std::ofstream::binary);
		prooffile << proof;
		prooffile.close();
	}
	...
	{
		std::ifstream ivk("V:\\06_GitHub\\02_bitbucket\\01_Quras\\quras-zk-snarks\\libsnark\\build_bn\\libsnark\\Debug\\proof", std::ofstream::binary);
		ivk >> proof;
		ivk.close();
	}

	r1cs_ppzksnark_primary_input<ppT> input;
	{
		std::ifstream ivk("V:\\06_GitHub\\02_bitbucket\\01_Quras\\quras-zk-snarks\\libsnark\\build_bn\\libsnark\\Debug\\primary_input", std::ofstream::binary);
		ivk >> input;
		ivk.close();
	}

	{
		std::ifstream ivk("V:\\06_GitHub\\02_bitbucket\\01_Quras\\quras-zk-snarks\\libsnark\\build_bn\\libsnark\\Debug\\vk.key", std::ofstream::binary);
		ivk >> pvk;
		ivk.close();
	}
