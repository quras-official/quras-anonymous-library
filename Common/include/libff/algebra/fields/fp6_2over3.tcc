...
template<mp_size_t n, const bigint<n>& modulus>
Fp6_2over3_model<n,modulus> Fp6_2over3_model<n,modulus>::Frobenius_map(unsigned long long power) const
{
...
Fp6_2over3_model<n, modulus> Fp6_2over3_model<n,modulus>::cyclotomic_exp(const bigint<m> &exponent) const
{
    ...
    std::vector<long long> NAF = find_wnaf(1, exponent);

    for (long long i = static_cast<long long>(NAF.size() - 1); i >= 0; --i)
    {
        if (found_nonzero)
        {
