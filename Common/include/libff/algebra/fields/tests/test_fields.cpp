...
void test_field()
{
    ...
    assert_except(a * a == a.squared());
    assert_except((a + b).squared() == a.squared() + a*b + b*a + b.squared());
    assert_except((a + b)*(c + d) == a*c + a*d + b*c + b*d);
    assert_except(a - b == a + (-b));
    assert_except(a - b == (-b) + a);

    assert_except((a ^ rand1) * (a ^ rand2) == (a^randsum));

    assert_except(a * a.inverse() == one);
    assert_except((a + b) * c.inverse() == a * c.inverse() + (b.inverse() * c).inverse());
...
void test_sqrt()
{
    for (size_t i = 0; i < 100; ++i)
    {
        ...
        assert_except(asq.sqrt() == a || asq.sqrt() == -a);
    }
}
...
{
    FieldT a = FieldT::random_element();
    assert_except(a.squared() == a * a);
    assert_except(a.squared() == a.squared_complex());
    assert_except(a.squared() == a.squared_karatsuba());
}
...
{
    assert_except(FieldT::extension_degree() % 2 == 0);
    FieldT a = FieldT::random_element();
    FieldT aqcubed_minus1 = a.Frobenius_map(FieldT::extension_degree()/2) * a.inverse();
    assert_except(aqcubed_minus1.inverse() == aqcubed_minus1.unitary_inverse());
}
...
void test_cyclotomic_squaring<Fqk<edwards_pp> >()
{
    ...
    assert_except(beta.cyclotomic_squared() == beta.squared());
}
...
void test_Fp4_tom_cook()
{
    ...
    for (size_t i = 0; i < 100; ++i)
    {
        ...
        assert_except(res == correct_res);

        // {v0, v3, v4, v5}
        const FieldT u = (FieldT::one() - beta).inverse();
        assert_except(v0 == u * c0 + beta * u * c2 - beta * u * FieldT(2).inverse() * v1 - beta * u * FieldT(2).inverse() * v2 + beta * v6);
        assert_except(v3 == - FieldT(15) * u * c0 - FieldT(30) * u * c1 - FieldT(3) * (FieldT(4) + beta) * u * c2 - FieldT(6) * (FieldT(4) + beta) * u * c3 + (FieldT(24) - FieldT(3) * beta * FieldT(2).inverse()) * u * v1 + (-FieldT(8) + beta * FieldT(2).inverse()) * u * v2
               - FieldT(3) * (-FieldT(16) + beta) * v6);
        assert_except(v4 == - FieldT(15) * u * c0 + FieldT(30) * u * c1 - FieldT(3) * (FieldT(4) + beta) * u * c2 + FieldT(6) * (FieldT(4) + beta) * u * c3 + (FieldT(24) - FieldT(3) * beta * FieldT(2).inverse()) * u * v2 + (-FieldT(8) + beta * FieldT(2).inverse()) * u * v1
               - FieldT(3) * (-FieldT(16) + beta) * v6);
        assert_except(v5 == - FieldT(80) * u * c0 - FieldT(240) * u * c1 - FieldT(8) * (FieldT(9) + beta) * u * c2 - FieldT(24) * (FieldT(9) + beta) * u * c3 - FieldT(2) * (-FieldT(81) + beta) * u * v1 + (-FieldT(81) + beta) * u * v2
